...
const SKIP_ENV_NEW = 'NUXT_DDD_TOOLKIT_SKIP_POSTINSTALL'
const SKIP_ENV_LEGACY = 'LARAVELIZE_SKIP_POSTINSTALL'
...
export function runPostinstall(projectRoot: string | undefined = process.env.INIT_CWD): void {
  if (process.env[SKIP_ENV_NEW] === '1' || process.env[SKIP_ENV_LEGACY] === '1') return
  if (projectRoot === undefined || projectRoot === '') return
  if (projectRoot === process.cwd()) return

  const logger = (line: string) => process.stdout.write(`nuxt-ddd-toolkit: ${line}\n`)

  try {
    linkSkills({ projectRoot, logger })
  } catch (error) {
    process.stderr.write(`nuxt-ddd-toolkit postinstall failed: ${(error as Error).message}\n`)
  }
}
